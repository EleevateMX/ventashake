-- =====================================================================
-- Corrige un hueco real encontrado al preparar la Fase 3 del costeo de
-- empaques: la función que SÍ corre automáticamente en cada guardado de
-- costosshake (fn_sync_app_data(), enganchada al trigger app_data_sync
-- en auto_sync_app_data_trigger.sql) nunca se actualizó con el arreglo
-- de empaques por receta — solo se corrigió una copia separada,
-- supabase/seed/sync-app-data.sql, que es un script MANUAL y no es lo
-- que realmente se ejecuta solo. Sin este cambio, aunque el operador ya
-- asigne los empaques correctos a cada receta en costosshake
-- (docs/empaques-por-receta.md), esas líneas nunca habrían llegado a
-- `recetas` por la vía automática — vw_costeo_producto seguiría viendo
-- costo_empaque = 0 para todos los productos.
--
-- Este archivo reemplaza fn_sync_app_data() con la misma lógica de
-- siempre MÁS los dos bloques de empaques (idénticos a los ya probados
-- en supabase/seed/sync-app-data.sql). Aditivo: create or replace, no
-- borra nada; los datos ya sincronizados no se tocan, solo se agregan
-- los que falten en el próximo guardado.
-- =====================================================================
...
begin
  ...
  -- Empaques por receta (el arreglo que faltaba en esta copia — ver
  -- comentario de cabecera). Igual que el bloque de proteína, valida por
  -- (producto, insumo) en vez de "el producto no tiene NINGUNA receta",
  -- así que si se agrega un empaque a un shake que ya tenía receta, el
  -- siguiente guardado sí lo agrega, sin duplicar los que ya existen.
  insert into recetas (producto_id,insumo_id,cantidad,nota)
