-- Corrige la causa de los productos duplicados en POS/Kiosko (reporte del
-- cliente con captura: "César Crunch" Y "Ensalada - César Crunch", siete
-- variantes de "Wrap Smoky Chipotle Chicken", etc.).
--
-- Causa raíz: la sincronización costosshake → productos empata por NOMBRE.
-- Al renombrar un producto en costosshake, el nombre nuevo no empata con
-- nada → se inserta un producto NUEVO, y el viejo se queda activo para
-- siempre. Cada intento de renombre del cliente dejó un duplicado activo
-- (se encontraron 26 alimentos activos cuando costosshake solo tiene 7).
--
-- Corrección de la relación de datos (no un parche visual): al final del
-- sync, cualquier producto de las categorías que costosshake ALIMENTA
-- (Shakes, Alimentos, Bebidas, Snacks) cuyo nombre ya no exista en el
-- documento se DESACTIVA (baja lógica — nunca se borra: conserva su
-- historial de órdenes/ventas). Los combos (es_combo) y las categorías
-- administradas desde Admin (Combos, Extras u otras nuevas) no se tocan.
--
-- Importante para la operación: en esas 4 categorías, costosshake es la
-- fuente de la verdad del catálogo — un producto creado a mano en
-- Admin → Menú dentro de ellas se desactivará en el siguiente guardado de
-- costosshake si no existe allá. Para productos manuales usar otras
-- categorías (p. ej. Extras).
--
-- Este archivo redefine fn_sync_app_data() completa (create or replace)
-- agregando el bloque de desactivación al final. El resto es idéntico a
-- costeo_fix_auto_sync_empaques.sql.
